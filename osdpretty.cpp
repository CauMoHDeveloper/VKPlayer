...
//Класс графического интерфейса уведомлений
//На вход подается две строки текста, картинка, и время показа уведомлений в мс.
...
const int OSDPretty::kMaxIconSize = 35;

const int OSDPretty::kSnapProximity = 20;

const QRgb OSDPretty::kPresetBlue = qRgb(49, 49, 49);
const QRgb OSDPretty::kPresetOrange = qRgb(47, 126, 165);
...
void OSDPretty::Reposition() {
  ...
  int x = popup_pos_.x() < 0 ? geometry.left() - width()
                             : geometry.right() + popup_pos_.x();
  int y = popup_pos_.y() < 0 ? geometry.top() - height()
                             : geometry.bottom() + popup_pos_.y();

  move(qBound(0, x, geometry.right() - width()),
       qBound(0, y-100, geometry.bottom() - height()));
...
void OSDPretty::set_background_color(QColor color) {
